...
| <a name="input_service_account_set_key_path"></a> [service\_account\_set\_key\_path](#input\_service\_account\_set\_key\_path) | Key path used in `helm --set` to point to the service account in the chart values (ex.: `serviceAccount` or `rbac.serviceAccount`) | `string` | `"serviceAccount"` | no |
